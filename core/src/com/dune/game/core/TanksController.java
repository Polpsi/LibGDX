...
public class TanksController extends ObjectPool<Tank> {
    ...
    public void setup(float x, float y, GameController.Owner ownerType) {
    ...
    public Tank getNearestAiTank(Vector2 point) {
        for (int i = 0; i < activeList.size(); i++) {
            ...
            if (t.getOwnerType() == GameController.Owner.AI && t.getPosition().dst(point) < 30) {
                return t;
            }
        }
        return null;
    ...
    public void playerUpdate(float dt) {
        if (Gdx.input.isButtonPressed(Input.Buttons.RIGHT)) {
            for (int i = 0; i < gc.getSelectedUnits().size(); i++) {
                ...
                if (t.getOwnerType() == GameController.Owner.PLAYER && gc.getSelectedUnits().contains(t)) {
